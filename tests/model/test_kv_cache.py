...
def _fixture_decoder(
    ...
    return TransformerDecoder(config=cfg, vocab_size_in=vocab_size, max_len=max_len).eval()
...
def test_step_matches_forward_full_sequence() -> None:
    ...
    full_out = decoder(input_ids, memory=memory)

    cache = None
    step_outputs = []
    for t in range(input_ids.shape[1]):
        step_in = input_ids[:, t : t + 1]
        out, cache = decoder.step(step_in, memory=memory, cache=cache, start_pos=t)
...
def test_step_after_prompt_matches_full() -> None:
    ...
    prompt = input_ids[:, :8]
    prompt_out, cache = decoder.step(prompt, memory=memory, cache=None, start_pos=0)
...
def test_cross_kv_cached_across_steps() -> None:
    ...
    for t in range(input_ids.shape[1]):
        step_in = input_ids[:, t : t + 1]
        _, cache = decoder.step(step_in, memory=memory, cache=cache, start_pos=t)
