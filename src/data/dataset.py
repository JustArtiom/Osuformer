...
class Dataset:
  ...
  def build_cache(self, name: str, limit: int = -1):
    ...
    for type_split, files in split.items():
      jobs = [(p, self.config, pwd) for p in files]
      with Pool(min(self.workers, cpu_count())) as pool:
        for i, (hash_id, audio_mel, tokens) in enumerate(tqdm(
          pool.imap_unordered(process_map_sample, jobs),
          total=len(jobs),
          desc=f"Processing {type_split} cache"
        )):
          if hash_id is None or audio_mel is None or tokens is None:
            continue
...
def process_map_sample(args: tuple[Path, ExperimentConfig, Path]) -> Union[tuple[str, np.ndarray, list[int]],tuple[None, None, None]]:
  osu_path, config, pwd = args
  ...
  return hash_id, audio_mel, tokens